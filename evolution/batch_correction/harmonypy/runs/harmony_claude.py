...
class Harmony:
    ...
    def __init__(
        ...
        # Pre-compute transpose for reuse in ridge regression
        ...
        # Pre-compute sigma reciprocal for repeated use
    ...
    def init_cluster(self, random_state):
        ...
        # Compute R using log-sum-exp for numerical stability
        self._compute_R_from_dist()
    ...
    def _compute_R_from_dist(self):
        """Compute soft cluster assignments from distance matrix using log-sum-exp."""
        log_R = -self._dist_mat * self._inv_sigma[:, None]
        # Log-sum-exp normalization for numerical stability
        log_R_max = log_R.max(dim=0, keepdim=True).values
        R_exp = torch.exp(log_R - log_R_max)
        self._R = R_exp / R_exp.sum(dim=0, keepdim=True)
    ...
    def compute_objective(self):
        ...
        theta_log = self._theta.unsqueeze(0) * torch.log(ratio)
    ...
    def cluster(self):
        ...
        for i in range(self.max_iter_kmeans):
            # Update Y: weighted centroid computation
    ...
    def update_R(self):
        # Compute scaled distances using log-sum-exp for stability
        log_scale = -self._dist_mat * self._inv_sigma[:, None]
        log_max = log_scale.max(dim=0, keepdim=True).values
        self._scale_dist = torch.exp(log_scale - log_max)
        self._scale_dist = self._scale_dist / self._scale_dist.sum(dim=0, keepdim=True)
        ...
        for blk in range(n_blocks):
            ...
            # Recompute R for this block with numerical stability
            O_E_sum = torch.clamp(self._O + self._E, min=1e-8)
            ratio = torch.clamp(self._E / O_E_sum, min=1e-8, max=1.0)
            # Vectorized power: ratio^theta using log-exp trick
            ratio_powered = torch.exp(self._theta.unsqueeze(0) * torch.log(ratio))
            R_block_new = scale_block * (ratio_powered @ Phi_block)
            R_block_new = R_block_new / torch.clamp(R_block_new.sum(dim=0, keepdim=True), min=1e-8)
    ...
    def moe_correct_ridge(self):
        ...
        Optimized: replaces inner B-loop with matrix multiply,
        uses torch.linalg.solve instead of explicit inverse.
        """
        # Accumulate total correction
        correction = torch.zeros_like(self._Z_orig)
        ...
        for k in range(self.K):
            ...
            Rk = self._R[k, :]
            Phi_Rk = self._Phi_moe * Rk

            # Compute covariance matrix with ridge penalty
            cov_mat = Phi_Rk @ self._Phi_moe_T + torch.diag(lamb_vec)

            # Compute right-hand side: Phi_moe @ diag(R[k,:]) @ Z_orig.T
            # = Phi_Rk @ Z_orig.T  (shape: (B+1) x d)
            rhs = Phi_Rk @ self._Z_orig.T

            # Solve for W: cov_mat @ W = rhs => W = cov_mat^{-1} @ rhs
